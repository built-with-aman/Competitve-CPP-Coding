...
// for deleting we have to create a destructor 
~linkedlist(){
  int value = this->data;
  // memory free
if(this->next != NULL){  // if next is not null then only we will delete it
  delete next;
  this->next = NULL;  // avoid dangling pointer because next is deleted so we will set it to null , dangling pointer is a pointer that does not point to a valid object
...
// ----------------------------------delete-------------------------------------------
void deletenode(linkedlist* &head, int nth) {
    if(nth == 1) {  // delete head
        linkedlist* temp = head;
        head = head->next;
...
// then we will have to add an condition 
 ...
 deletenode(head, 2); // deletes the 2nd node
print(head);
